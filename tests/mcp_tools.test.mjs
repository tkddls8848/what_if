...
test("evidence_for quotes text that really exists in the segment", () => {
  ...
  const result = evidenceForFact(library, { document_id: "gamja", fact_id: event.event_id });
...
test("evidence_for rejects unknown fact ids", () => {
  assert.throws(() => evidenceForFact(library, { document_id: "gamja", fact_id: "event_99999" }), ToolError);
  assert.throws(() => evidenceForFact(library, { document_id: "gamja", fact_id: "nonsense" }), ToolError);
});
