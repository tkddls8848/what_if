...
| `/` | 플레이 기본 진입점 (`/play`와 동일 화면) |
| `/worlds` | 세계관·캐릭터 목록과 새 이야기 시작 |
| `/check` | 기존 카드의 이름·성격·금기·말투·지식 시점 검수·수정·제외 |
| `/session` | 세션 보관함, 이어하기, 턴별 스냅샷, 분기, Markdown·JSON 내려받기 |

읽는 화면의 관계·회복 표시는 접힌 상세 영역에 둔다. Neuron 잔량은 서버가 오늘
집계한 추정치이며 세션별 할당이 아니다. `/session`은 관계·회복·장소 스냅샷을
보여준다. 사건·지식 그래프와 감사, RAG는 아직 구현하지 않았다.
`/worlds`에서는 설정 본문 또는 Fandom URL을 로컬 Ollama로 카드 컴파일한다.
약 1,000자 청크에서 반복 확인된 인물·설정과 실제 인용만 합치고 관계를 별도로
추출한다. 결과는 `suggested`로 저장해 `/check`에서 확정하며, 그 전에는 플레이에서
제외한다. 기존 카드는 덮어쓰지 않는다. 원자료는 세계관 파일의 `sources`에 보관한다.
분기는 저장된 턴 스냅샷만 사용하며 미래 상태를 복사하지 않는다. 스냅샷이 없는
턴의 분기는 거부한다. 사용자가 종료한 세션은 새로고침 후에도 종료 상태를 유지한다.
...
여기서부터는 `/analyze`와 `/analyze/check`에 남아 있는 분석기의 설계다. 코드는 그대로 동작하고
